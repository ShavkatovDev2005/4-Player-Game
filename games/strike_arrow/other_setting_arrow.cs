...
public class other_setting_arrow : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (can_ver>=7)
        {
            ...
            random_y_can = Random.Range(3f, -3f);

            GameObject health = Instantiate(random_can, transform);
            health.transform.localPosition = new Vector3(random_x_can,random_x_can,1);
            can_ver=0;
        }
        ...
        if (mermiHesap < 4)
        {
            ...
            random_y = Random.Range(-3f, 3f);
            
            // Yeni bir mermi oluştur
            GameObject bullet = Instantiate(random_mermi, transform);
            bullet.transform.localPosition = new Vector3(random_x,random_y,1);
    ...
    public void FireBullet(Transform player)//mermiler buradan verilir
    {
        ...
        bulletRb.linearVelocity = player.transform.right * 10f;  // Merminin hareket hızı
    }
