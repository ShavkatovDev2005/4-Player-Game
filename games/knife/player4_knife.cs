...
public class player4_knife : MonoBehaviour
{
    public static player4_knife Instance;
    ...
    void Start()
    {
        selected_knife = PlayerPrefs.GetInt("knife4",0);
    ...
    public void buttonExit()
    {
        ...
        if (imkoniyat<5 && hayatta) 
        {
            ...
            shootable.transform.rotation = Quaternion.Euler(0,0, 64);
            Rigidbody2D rb = shootable.GetComponent<Rigidbody2D>();
            Collider2D col = shootable.GetComponent<Collider2D>();
            shootable.GetComponent<knife>().hangi_oyuncu_atti=4;
