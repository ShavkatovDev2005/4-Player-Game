...
public class karakters : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        tank_string= PlayerPrefs.GetString("tank_string","11110000000000");//14 tank
        ...
        for (int i = 0; i < 4; i++)
        {
            players_material[i].color = transform.GetChild(1).GetChild(PlayerPrefs.GetInt("color"+(i+1).ToString(),i)).GetComponent<Image>().color;
        }
        load_color();
...
#region tank
    public void tank_select(int secilen)
    {
        ...
        int[] price = {  0 ,  0 , 0 , 0 , 2000 , 2000 , 2000 , 5000 , 5000 , 5000 , 5000 , 5000 , 10000 , 10000};  // price
        if (secilen < 0 || secilen >= price.Length) return;  // Geçerli secilen değerini kontrol et

        if (PlayerPrefs.GetInt("Coin", 0) >= price[secilen] || tank_string[secilen]=='1')
        {
            // Seçilen tankı güncelle
            PlayerPrefs.SetInt("tank"+player.ToString(),secilen);
            tank_selected();
...
#region knife
    public void knife_select(int secilen)
    {
        ...
        int[] price = {  0 ,  0 , 0 , 0 , 2000 , 2000 , 2000 , 5000 , 5000 , 10000 , 10000};  // price
        if (secilen < 0 || secilen >= price.Length) return;  // Geçerli secilen değerini kontrol et

        if (PlayerPrefs.GetInt("Coin", 0) >= price[secilen] || knife_string[secilen]=='1')
        {
            // Seçilen tankı güncelle
            PlayerPrefs.SetInt("knife"+player.ToString(),secilen);
            knife_selected();
...
# region color
    public void color_select(int secilen)
    {
        ...
        int[] price = {  0 ,  0 , 0 , 0 , 5000 , 5000 , 10000 , 10000 , 15000 , 15000 , 20000 , 20000};  // price
        if (secilen < 0 || secilen >= price.Length) return;  // Geçerli secilen değerini kontrol et

        if (PlayerPrefs.GetInt("Coin", 0) >= price[secilen] || color_string[secilen]=='1')
        {
            for (int i = 0; i < 4; i++)// Agar boshqa o‘yinchilar tanlangan rang bilan birxil bo‘lsa, ularni rangini o‘zgartiramiz
            {
                if (i != player - 1 && players_material[i].color == transform.GetChild(1).GetChild(secilen).GetComponent<Image>().color)
                {
                    PlayerPrefs.SetInt("color"+(i+1),PlayerPrefs.GetInt("color"+player.ToString(),player-1));
                    players_material[i].color = transform.GetChild(1).GetChild(PlayerPrefs.GetInt("color"+player.ToString(),player-1)).GetComponent<Image>().color;
                }
            }

            // Seçilen color güncelle
            PlayerPrefs.SetInt("color"+player.ToString(),secilen);
            color_selected();
    ...
    void color_selected()//colordaki ptichkani to'g'irlaydi
    {
        ...
        // O‘yin menyusidagi player button ranglarini yangilash
        for (int i = 0; i < 4; i++)
        {
